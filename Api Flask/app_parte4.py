...
#clase inventario contiene producto
class Inventario:
  def __init__(self):
  ...
  def agregar_producto(self, codigo, descripcion, cantidad, precio):
    ...
    nuevo_producto = Producto(codigo, descripcion, cantidad, precio) #corroborar si esta linea esta de mas###############
...
app = Flask(__name__) #creo instancia de Flask que se llama app
CORS(app)
...
@app.route('/productos/<int:codigo>', methods=['GET'])#antes de la '/productos' va la url de pythonanywhare
def obtener_producto(codigo):
  ...
  if producto:
    ...
    }), 200 #Estatus 200 es q esta todo bien
  return jsonify({'message': 'Producto no encontrado.'}), 404

# Ruta para obtener la lista de productos del inventario
@app.route('/') # aca va la URL 
def index():
  return 'API de Inventario'

# Ruta para obtener la lista de productos del inventario
@app.route('/productos', methods=['GET']) #poner mi URL. Es el read
def obtener_productos():
  return inventario.listar_productos()

# Ruta para agregar un producto al inventario
@app.route('/productos', methods=['POST']) #post agrega el producto, es el create
...
@app.route('/carrito', methods=['POST'])#create
def agregar_carrito():
  codigo = request.json.get('codigo')
  cantidad = request.json.get('cantidad')
